...
class YoubotKinematicStudent(YoubotKinematicBase):
    ...
    def get_jacobian(self, joint):
        ...
        for i in range(5):
            ...
            jacobian[:3, i] = np.cross(z_prev, (o_e - o_prev))
            # Angular velocity part
            jacobian[3:, i] = z_prev

            # Update for next joint
            z_i = T[:3, 2] # Extract the z-axis
            o_i = T[:3, 3] # Extract the origin
            z_prev = z_i
            o_prev = o_i
    ...
    def check_singularity(self, joint):
        ...
        # TODO: Implement this
        # Your code starts here ----------------------------
        # Matrix Rank: A matrix is said to be singular if its rank is less than its full rank.

        singularity = False
        jacobian = self.get_jacobian(joint)
        
        # Check if the Jacobian matrix is singular: np.linalg.matrix_rank computes the 
        # rank of the matrix based on a tolerance
        if np.linalg.matrix_rank(jacobian, tol=1e-16) < len(joint):
            singularity = True
