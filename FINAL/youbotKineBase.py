...
class YoubotKinematicBase(Node):
    ...
    def standard_dh(a, alpha, d, theta):
        ...
        # The transformation matrix A is computed using the DH parameters a, alpha, d, and theta
        A = np.array([
        ...
        assert isinstance(A, np.ndarray), "Output wrong type (ndarray)"
        assert A.shape == (4, 4), "Output wrong dimensions (4x4)"
        return A
	...
	to rodrigues.
        ...
        q = rotmat2q(T[:3, :3])
        ang = 2 * np.arccos(q.w) # Calculate the rotation angle (ang)
        ang = np.mod(ang + np.pi, 2 * np.pi) - np.pi # if want to keep rotation angle range between [-np.pi, np.pi]
        norm_vector = np.linalg.norm([q.x, q.y, q.z])

	    # If the quaternion represents no rotation, return a zero vector
        if np.isclose(ang, 0):
            rod_vec = np.zeros(3)
        else:
            rod_vec = ang * np.array([q.x, q.y, q.z]) / norm_vector # Calculate the Rodrigues vector (r = ang * u) and the unit rotation axis (u)

        p = np.zeros(6)
        p[:3] = T[:3, 3]  # Translation
        p[3:] = rod_vec  # Rodrigues vector
        return p
